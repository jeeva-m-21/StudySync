...
class CourseManagementUI:
    def __init__(self, master, username):
        ...
        # Set background color to match the login UI
        self.master.config(bg="#282c34")

        # Title
        title_label = tk.Label(master, text="Manage Your Courses", font=("Helvetica", 16), bg="#282c34", fg="white")
        title_label.pack(pady=10)

        # Frame to display courses
        self.course_frame = tk.Frame(master, bg="#3c4043")
        self.course_frame.pack(pady=5, padx=20)

        # Load and display courses
        self.display_courses()

        # Buttons for adding and managing courses
        self.add_course_button = tk.Button(master, text="Add Course", command=self.add_course, bg="#4CAF50", fg="white", font=("Helvetica", 12), relief="flat")
        self.add_course_button.pack(pady=5, fill=tk.X, padx=20)

        self.delete_course_button = tk.Button(master, text="Delete Course", command=self.delete_course, bg="#e74c3c", fg="white", font=("Helvetica", 12), relief="flat")
        self.delete_course_button.pack(pady=5, fill=tk.X, padx=20)
    ...
    def display_courses(self):
        ...
        for course in courses:
            course_button = tk.Button(self.course_frame, text=course, command=lambda c=course: self.manage_course(c), bg="#2196F3", fg="white", font=("Helvetica", 12), relief="flat")
            course_button.pack(pady=2, fill=tk.X)
    ...
    def manage_course(self, course):
        ...
        course_window.configure(bg="#3c4043")

        # Header Label
        tk.Label(course_window, text=f"You are managing the course: {course}", bg="#3c4043", fg="white", font=("Helvetica", 14)).pack(pady=15)
        ...
        def manage_notes():
            notes.start_notes_app(self.username)

        def add_materials():
            materials.run_materials(self.username)

        def add_recording():
            recordings.recordrunner(self.username)  # Placeholder for actual functionality

        def manage_todo():
            todo.todorunner(self.username)
